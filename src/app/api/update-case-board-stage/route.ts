...
export async function POST(request: NextRequest) {
  try {
    ...
    const { caseId, boardStage } = body;

    if (!caseId || !boardStage) {
      return NextResponse.json(
        { error: 'Case ID and board stage are required' },
        { status: 400 }
      );
    }
    ...
    const { data, error } = await supabaseAdmin
      .from('cases')
      .update({ board_stage: boardStage })
      .eq('id', caseId)
      .select()
      .single();

    if (error) {
      console.error('Error updating case board stage:', error);
      return NextResponse.json(
    ...
    console.error('Error in update-case-board-stage API:', error);
    return NextResponse.json(
