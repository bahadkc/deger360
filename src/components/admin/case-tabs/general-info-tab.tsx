...
export function GeneralInfoTab({ caseData, onUpdate }: GeneralInfoTabProps) {
  ...
  const [hasAuthUser, setHasAuthUser] = useState<boolean | null>(null); // Track if customer has auth user in Supabase
  ...
  // Check if customer has required fields for password generation
  const hasRequiredFieldsForPassword = () => {
    const hasPlate = vehicleData.vehicle_plate && vehicleData.vehicle_plate.trim() !== '';
    const hasSurname = customerData.full_name && customerData.full_name.trim() !== '';
    return hasPlate && hasSurname;
  };

  // Auto-fill password with plaka.soyisim format
  const autoFillPassword = () => {
    if (hasRequiredFieldsForPassword()) {
      const password = getCurrentPasswordFormat();
      setPasswordData({ newPassword: password });
    }
  };
  ...
  useEffect(() => {
    if (caseData) {
      ...
      // Load password from Supabase user_auth table and check if auth user exists
      const loadPasswordFromSupabase = async () => {
        if (customerId) {
          try {
            const customerEmail = caseData.customers?.email || '';
            
            // First check if auth user exists in auth.users table (real check)
            let canLogin = false;
            if (customerEmail) {
              try {
                const authCheckResponse = await fetch(`/api/check-customer-auth?email=${encodeURIComponent(customerEmail)}`, {
                  method: 'GET',
                  credentials: 'include',
                });
                ...
                if (authCheckResponse.ok) {
                  ...
                  canLogin = authCheckData.canLogin || false;
                }
              } catch (authCheckError) {
                console.error('Error checking auth status:', authCheckError);
                // Fallback to user_auth check
              }
            }
            
            // Also load password from user_auth table
            ...
            // Set hasAuthUser based on real auth check
            setHasAuthUser(canLogin);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Update password if provided OR auto-generate if plaka and soyisim are available
      let passwordUpdateSuccess = true;
      let passwordUpdateError = null;
      
      // Determine password to use: provided password or auto-generated from plaka.soyisim
      const providedPassword = passwordData.newPassword?.trim();
      let passwordToUse = providedPassword;
      let wasPasswordAutoGenerated = false;
      
      // If password is empty but plaka and soyisim are available, auto-generate
      if (!passwordToUse && hasRequiredFieldsForPassword()) {
        passwordToUse = getCurrentPasswordFormat();
      ...
      if (passwordToUse && passwordToUse.length >= 6) {
        try {
          ...
          if (!customerEmail) {
            ...
            const englishPassword = turkishToEnglish(passwordToUse);
            ...
            if (!passwordResponse.ok || !passwordData_result.success) {
              ...
              setPasswordWasUpdated(true);
              setHasAuthUser(true); // Mark that auth user now exists
      ...
      if (passwordToUse && passwordToUse.length >= 6) {
        if (passwordUpdateSuccess) {
          if (wasPasswordAutoGenerated) {
            alert(`Bilgiler başarıyla kaydedildi. Şifre otomatik olarak "${passwordToUse}" formatında oluşturuldu ve Supabase'e kaydedildi.`);
          } else {
              ...
              <div className="space-y-2">
                <div className="flex gap-2">
                  <Input
                    ...
                    placeholder={hasRequiredFieldsForPassword() ? getCurrentPasswordFormat() : "Şifre girin"}
                    className="font-mono flex-1"
                  />
                  {hasRequiredFieldsForPassword() && (
                    <Button
                      type="button"
                      variant="outline"
                ...
                {hasAuthUser === false && !hasRequiredFieldsForPassword() && (
                  <div className="p-3 bg-red-50 border-2 border-red-300 rounded-lg">
                    <p className="text-sm font-semibold text-red-800 flex items-center gap-2">
                      <span>⚠️</span>
                      <span>Müşteri portalına giriş yapamaz - şifre belirleyin</span>
                    </p>
                    <p className="text-xs text-red-600 mt-1">
                      Plaka ve soyisim bilgilerini tamamlayın veya manuel olarak şifre girin.
                    </p>
                  </div>
                )}
                {hasAuthUser === false && hasRequiredFieldsForPassword() && (
                  <div className="p-3 bg-yellow-50 border-2 border-yellow-300 rounded-lg">
                    <p className="text-sm font-semibold text-yellow-800 flex items-center gap-2">
                      <span>ℹ️</span>
                      <span>Şifre otomatik oluşturulacak: {getCurrentPasswordFormat()}</span>
                    </p>
                    <p className="text-xs text-yellow-600 mt-1">
                      Kaydet butonuna bastığınızda şifre otomatik olarak "{getCurrentPasswordFormat()}" formatında oluşturulacak ve Supabase'e kaydedilecektir.
                    </p>
                  </div>
                )}
                <p className="text-xs text-neutral-500">
                  Şifreyi düzenleyebilir, silebilir veya üzerine yazabilirsiniz. Boş bırakılırsa ve plaka/soyisim varsa otomatik oluşturulur. En az 6 karakter olmalıdır.
                </p>
              </div>
            ) : (
              <div className="space-y-2">
                ...
                {hasAuthUser === false && (
                  <div className="p-3 bg-red-50 border-2 border-red-300 rounded-lg">
                    ...
                    <p className="text-xs text-red-600 mt-1">
                      Bu müşterinin Supabase'de auth kullanıcısı yok. Düzenle butonuna basarak plaka ve soyisim bilgilerini tamamlayın veya manuel olarak şifre belirleyin.
                    </p>
                  </div>
                )}
              </div>
